...
## Drill Down (Domains)
Diagnostics are organized into domains so you can focus on what matters. Use `qpane.DiagnosticsDomain` members (or their string values) to toggle them. The `DiagnosticsDomain` enum provides stable names for these toggles when hosts prefer typed configuration over raw strings.

### Memory & Resources
* **Cache (`DiagnosticsDomain.CACHE`):** Enable this domain when the host needs detailed memory usage, tile cache, pyramid cache, and mask cache rows.
    * *What to look for:* While the HUD tracks your total budget, enabling this domain adds rows for active render caches, including tiles, pyramids, and masks when those systems are in use.
* **Swap (`DiagnosticsDomain.SWAP`):** Enable this domain when the host needs navigation, prefetch, and renderer queue detail.
...
### Performance & Health
* **Executor (`DiagnosticsDomain.EXECUTOR`):** Enable this domain when the host needs worker counts, queue depth, and rejection detail.
    * *What to look for:* `Queued` tasks and `Threads` usage. If `Executor|Rejections` climbs, you might be hitting concurrency limits.
* **Retry (`DiagnosticsDomain.RETRY`):** Enable this domain when the host needs per-resource retry counts for unstable I/O.
    * *What to look for:* Active retries for tiles or pyramids. A high count here usually means I/O trouble.

### Feature Tools
* **Masks (`DiagnosticsDomain.MASK`):** Enable this domain when the host needs mask autosave, brush, and mask job rows.
    * *What to look for:* `Mask Autosave` status (pending saves), `Mask Jobs` (generation queues), and `Mask|Brush` details.
* **SAM (`DiagnosticsDomain.SAM`):** Enable this domain when the host needs predictor readiness, embedding cache, and worker rows.
...
* **Check state:** `QPane.diagnosticsDomainEnabled(domain)` returns `True` if active and raises for unavailable domains.
* **Toggle:** `QPane.setDiagnosticsDomainEnabled(domain, enabled)` turns a section on or off and emits the domain toggle signal.
...
* `QPane.diagnosticsDomainToggled`: Emits `(domain: str, enabled: bool)` when a specific section changes so host menus stay synchronized.
